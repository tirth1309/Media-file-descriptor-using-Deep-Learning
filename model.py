...
def get_video_test_data(video_feat_path):
    ...
    test_data = '/TestFeatures/_0nX-El-ySo_83_93.avi.npy'
    return test_data
...
#	loss_to_draw_epoch = []
        ...
        for start, end in zip(range(0, len(current_train_data), batch_size),
            ...
            current_feats_vals = list(map(lambda feature: np.load(feature), current_videos))
...
def test(model_path='./models/'):
    ...
    #print(test_data)
    test_videos = './TestFeatures/_0nX-El-ySo_83_93.avi.npy'
    ...
    sess = tf.InteractiveSession(config=config)
    sess.run(tf.global_variables_initializer())	
    saver.restore(sess, os.path.join(model_path, 'model-20'))
    ...
    video_feat = np.load('./TestFeatures/_0nX-El-ySo_83_93.avi.npy', allow_pickle = True)
